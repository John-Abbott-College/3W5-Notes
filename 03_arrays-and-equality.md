...
Using this code as your base:
```js
 let users = [{name:"tara", giftcardBalance: 123},
              {name:"bob", giftcardBalance: 63}];
        
let item = {name:"ball", isExclusive: true};

//returns true if the user can buy the item, false otherwise
function canBuy( user, item){
}
```
Add the following properties to each user:
- `loyaltyTier`: This determines the loyalty level of your user. (e.g. `"Gold"`, `"Silver"`, `"Bronze"`)
- `isPremium`: This determines whether your user is a premium user.
